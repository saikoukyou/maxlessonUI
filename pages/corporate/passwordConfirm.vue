...
<section v-if="show_pass" class="forgetPassWrap">
  ...
  <n-form ref="formRef" :model="form" :rules="rules" size="large">
    ...
    <n-form-item :show-label="false" path="password_confirmation">
      ...
      <n-button type="submit" name="submit" @click="submitForm" :loading="loading" value="パスワード登録">
        <span>パスワード登録</span>
      </n-button>
  </n-form>
</section>
...
const checkCode = async () => {
  let {
    data,
    error
  } = await useBusinessCodeCheckApi(form.code)

  if (data?.value && data.value.err == true) {
...
const submitForm = () => {
  formRef.value.validate(async (errors) => {
    ...
    let {
      ...
      error,
      refresh
    } = await useBusinessResetPasswordApi(form)
    ...
    if (data?.value && data.value.err == false) {
      ...
      let interval = setInterval(() => {
        if (timer.value === 0) {
          ...
          navigateTo("/corporate/login", { replace: true })
        } else {
          timer.value --
          console.log(timer)
        }
...
@media screen and (max-width: 520px) {
  .forgetPassWrap{width: 80%; !important;}
