<template>
	<NuxtLayout name="teacherout">
      ...
      <div class="apply col gap">
        <n-form  ref="formRef" :model="form" :rules="rules" :label-width="150" size="large">
          <n-grid :cols="24" :x-gap="24" class="responsive-grid">
            ...
            <n-form-item-gi :span="12" label="擅长的话题领域" path="topic">
              <n-select v-model:value="form.topic" :options="topicOptions" placeholder="请选择" />
            </n-form-item-gi>
            ...
            <n-form-item-gi :span="12" label="外语" path="other_language">
      ...
      <div class="bottom">
        <router-link to="/teacher/company"><span>公司信息</span>
        </router-link>
      </div>
		</div>
	</NuxtLayout>
...
if (applyInfo.value != null) {
  ...
  topicOptions.value = Object.keys(applyInfo?.value.topic).map(
      key => ({
        label: applyInfo?.value.topic[key],
        value: key
      })
  );
}
